...
class FileReader(FileReaderBase):
    ...
    @classmethod
    def generate_id_stay_point(cls, df, session_time_interval, session_distance_intervel, do_label_encode=True, only_last_metric=True):
        # check if data\tky\preprocessed\sample.csv exists
        # if not, read the raw data and preprocess it
        # if not osp.exists(osp.join(cls.root_path, 'preprocessed', 'sample.csv')):
        df = df.sort_values(by=['UserId', 'UTCTimeOffset'], ascending=True)
        
        df['last_latitude'] = df.groupby('UserId')['Latitude'].shift(1)
        df['last_longitude'] = df.groupby('UserId')['Longitude'].shift(1)
        ...
        df['time_interval'] = df['UTCTimeOffset'].diff().dt.total_seconds() / 60
        
        # else:
        #     df = pd.read_csv(osp.join(cls.root_path, 'preprocessed', 'sample.csv'), parse_dates=['UTCTimeOffset'], date_parser=lambda x: pd.to_datetime(x, format='%Y-%m-%d %H:%M:%S'))
        #     df = df.sort_values(by=['UserId', 'UTCTimeOffset'], ascending=True)
        ...
        for user, time_diff, distance_diff in tqdm(zip(df['UserId'], df['UTCTimeOffset'].diff(), df['distance_to_last'])):
            if start_user != user:
                start_id += 1
                start_user = user
            if pd.isna(time_diff) or pd.isna(distance_diff):
                time_interval.append(None)
                distance_interval.append(None)
                pseudo_session_trajectory_id.append(start_id)
                continue
            elif time_diff.total_seconds() / 60 > session_time_interval or distance_diff > float('inf'):
                start_id += 1
            time_interval.append(time_diff.total_seconds() / 60)
            distance_interval.append(distance_diff)
            pseudo_session_trajectory_id.append(start_id)
        ...
        for user, user1, user2, pseudo_session_trajectory_id1, pseudo_session_trajectory_id2 in zip(
            ...
            df['UserId'].shift(-1),
            pseudo_session_trajectory_id,
            pseudo_session_trajectory_id[1:] + [None]
        ):
            if user != user1 and user != user2:
                assert pseudo_session_trajectory_id1 != pseudo_session_trajectory_id2
